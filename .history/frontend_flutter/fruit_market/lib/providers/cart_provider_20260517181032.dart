...
class CartProvider extends ChangeNotifier {
  ...
  // ========== SỬA LẠI: Xóa các item được chọn theo danh sách productId ==========
  Future<void> removeSelectedItemsByIds(List<String> productIds) async {
    ...
    try {
      // Xóa trực tiếp từ service, không qua removeItem để tránh gọi refreshCart nhiều lần
      for (final productId in productIds) {
        print('🔍 Đang xóa sản phẩm: $productId');
        await _cartService.removeFromCart(productId);
      }
      
      // Load lại giỏ hàng một lần duy nhất sau khi xóa hết
      await loadCart(forceRefresh: true);
      
      _setLoading(false);
      print('✅ Đã xóa ${productIds.length} sản phẩm khỏi giỏ');
    } catch (e) {
      print('❌ Remove selected items by ids error: $e');
