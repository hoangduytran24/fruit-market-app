...
import '../services/voucher_service.dart'; // Sửa lại đường dẫn đúng
...
class AdminVoucherProvider with ChangeNotifier {
  ...
  // Kiểm tra mã voucher đã tồn tại chưa (case-insensitive)
  // excludeId: dùng để bỏ qua chính voucher đang sửa khi kiểm tra
  bool isVoucherCodeExists(String code, {String? excludeId}) {
    return _vouchers.any((v) => 
      v.voucherCode.toUpperCase() == code.toUpperCase() && 
      v.voucherId != excludeId
    );
  }
