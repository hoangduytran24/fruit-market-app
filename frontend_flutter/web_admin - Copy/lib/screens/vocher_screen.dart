...
class _VouchersScreenState extends State<VouchersScreen> {
  ...
  String? _duplicateCodeError; // Lưu lỗi trùng mã real-time
  String? _editingVoucherId; // Lưu id của voucher đang sửa để tránh tự trùng
  ...
  void initState() {
    ...
    // Thêm listener để kiểm tra trùng mã real-time
  ...
  // Hàm kiểm tra trùng mã real-time
  ...
  void _openVoucherDialog(Voucher? voucher) {
    ...
    // Lưu id voucher đang sửa để kiểm tra trùng
    _editingVoucherId = isEdit ? voucher.voucherId : null;
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildCard([
                      // TextField với hiển thị lỗi real-time
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          _buildTextField(
                          ...
                          // Hiển thị cảnh báo trùng mã real-time
  ...
  Widget _buildTextField({
    ...
    return TextFormField(
      ...
      decoration: InputDecoration(
        ...
        // Thêm hiệu ứng khi có lỗi trùng
        ...
        Expanded(
          ...
          child: ElevatedButton(
            onPressed: _duplicateCodeError != null
                ? null // Vô hiệu hóa nút nếu đang có lỗi trùng mã
                : () async {
                    ...
                    try {
                      ...
                      if (mounted) {
                        // Nếu backend vẫn báo lỗi trùng (trường hợp dữ liệu chưa refresh)
                        if (e.toString().contains("Mã voucher đã tồn tại")) {
                          setState(() {
                            _duplicateCodeError = "Mã voucher đã tồn tại! Vui lòng chọn mã khác.";
                          });
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                            content: const Text("Mã voucher đã tồn tại, vui lòng nhập mã khác"), 
