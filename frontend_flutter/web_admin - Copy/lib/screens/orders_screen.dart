...
class _OrdersScreenState extends State<OrdersScreen> {
  ...
  // Nên shippingFee chỉ dùng để hiển thị UI, không cộng thêm khi tính tổng
  ...
  Widget _buildOrderRow(OrderListDto order) {
    // Backend đã cộng phí ship 25k vào finalAmount, nên hiển thị trực tiếp
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
              flex: 2,
              child: Text(_formatCurrency(order.finalAmount), // Đã bao gồm ship
...
class _OrderDetailDialog extends StatelessWidget {
  ...
  // Lưu ý: Backend đã cộng phí ship 25k vào totalAmount
  // Nên để hiển thị đúng, ta cần tách tiền hàng và phí ship
  ...
  double _getFinalTotalWithShipping() {
    // Tổng thanh toán = totalAmount (đã bao gồm ship)
  ...
  Widget build(BuildContext context) {
    ...
    final double productTotal = _getProductTotal(); // Tiền hàng chưa ship
    final double finalTotal = _getFinalTotalWithShipping(); // Đã bao gồm ship
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    ...
                    // Hiển thị số lượng sản phẩm và tổng số lượng
                    Container(
                    ...
                    // Chi tiết giá - đã tách tiền hàng và phí ship
                    _buildReceiptRow("Tổng tiền hàng:", formatCurrency(productTotal)),
                    if(order.discountAmount > 0)
                      _buildReceiptRow("Giảm giá:", "-${formatCurrency(order.discountAmount)}", color: Colors.red),
                    
                    _buildReceiptRow(
                      "Tạm tính (sau giảm giá):", 
                      formatCurrency(productTotal - (order.discountAmount ?? 0)),
                      isBold: true,
                    ),
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          ...
                          Text("Phương thức: ${order.paymentMethod?.toUpperCase() ?? 'COD'}", 
                  ...
                  if (isCompleted) 
                    Expanded(
                      ...
                      child: ElevatedButton.icon(
                        onPressed: () async {
                          // Backend đã cộng phí ship 25k vào finalAmount
                          // Nên khi xuất hóa đơn, finalAmount đã đúng
